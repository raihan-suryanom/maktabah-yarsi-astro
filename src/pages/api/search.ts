...
export const POST: APIRoute = async ({ request, redirect }) => {
  try {
    ...
    if (!hasQuery) {
      ...
      data.set("entry", formData.get("entry")!);
    }
    ...
    if (!data.has("open")) data.append("open", "true");
    ...
    if (data.has("id")) {
      data.set("id", formData.has('id') ? formData.get('id')! : result[0].bibliography);
    } else {
      data.append("id", result[0].bibliography);
    }

    if (data.has("page")) {
      data.set("page", formData.has('page') ? formData.get('page')! : result[0].page.toString());
    } else {
